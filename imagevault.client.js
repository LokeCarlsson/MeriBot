...
client.prototype = {
	...
	_ajaxWithCorsSupport: function (config) {
		...
		if (config.processData && (typeof config.data === "object" || (typeof config.data==="string" && config.data.length>0 && config.data.charAt(0)!='{' && config.data.charAt(0)!='['))) {
		...
		var options = {uri:config.url,method:config.type,headers:config.headers};
		if(config.data) {
		...
		if(config.form) {
			// console.log("Form data:"+config.form);
			options.form=config.form;
		}
		request(options, function(error,response,body) {
			if(error) {
				config.error(response,null,error);
			} else{
	...
	_getAuthToken: function (callback) {
		...
		var config = {
			...
			error: function (xhr, status, err) {
				...
				self._lastError = { status: xhr, message: err };
					callback(null);
				}
			};
